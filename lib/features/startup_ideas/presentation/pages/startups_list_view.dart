...
import '../../../founder_dashboard/presentation/pages/my_startup_view.dart';
...
class _StartupsListViewState extends State<StartupsListView> {
  ...
  Future<void> _bootstrap() async {
    ...
    await _fetchMyStartup();
  }

  Future<void> _fetchMyStartup() async {
    final repo = sl<StartupRepository>();
    final myRes = await repo.getMyStartups(
      const QueryParams(page: 1, pageSize: 1),
    );
    if (mounted) {
      setState(() {
        _myStartup = myRes.valueOrNull?.items.firstOrNull;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: !isExplore
              ? const MyStartupView(showBackButton: false)
              : CatalogView<Startup>(
                  ...
                  floatingActionButton: Builder(
                    builder: (fabContext) => FloatingActionButton.extended(
                      onPressed: () => _postStartup(
                        fabContext,
                        repo,
                        initialData: _myStartup,
                      ),
                      icon: Icon(
                        _myStartup == null
                            ? Icons.add_rounded
                      ...
                      label: Text(
                        _myStartup == null ? 'Post Startup' : 'Edit Startup',
                      ),
                    ...
                    return AppStartupCard(
                      ...
                      onInterest: () async {
                        if (s.hasInvested) {
                          ...
                          res.fold(
                            (f) =>
                                context.showTopSnack(f.message, isError: true),
                            (success) {
                              if (success) {
                                setState(() => _refreshKey++);
                                context.showTopSnack(
  ...
  Future<void> _postStartup(
    ...
    final data = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (ctx) => EditIdeaBottomSheet(
        startup:
            initialData ??
            const Startup(
              id: '',
    ...
    final res = initialData != null
        ? await repo.updateIdea(initialData.id, data)
        : await repo.createIdea(data);
    ...
    if (context.mounted) {
      res.fold((f) => context.showTopSnack(f.message, isError: true), (
        ...
        context.showTopSnack(
          initialData != null
              ? 'Startup Idea updated successfully!'
