...
class BookmarksPage extends StatelessWidget {
  ...
  /// Fetch saved projects based on auth token
  Future<Result<Paginated<Project>>> _fetchProjects(QueryParams params) async {
    ...
    return client.getEnvelope<Paginated<Project>>(
  ...
  /// Fetch saved freelancers based on auth token
  Future<Result<Paginated<Freelancer>>> _fetchFreelancers(
    ...
    return client.getEnvelope<Paginated<Freelancer>>(
      ApiEndpoints.publicSavedFreelancers,
      query: query,
      parser: (env) {
        final list = env.data as List? ?? [];
  ...
  Future<Result<Paginated<Startup>>> _fetchStartups(QueryParams params) async {
    ...
    return client.getEnvelope<Paginated<Startup>>(
      ApiEndpoints.publicSavedStartups,
      query: query,
      parser: (env) {
        final list = env.data as List? ?? [];
        final items = list
            .map(
              (e) => Startup.fromApiJson(
                Map<String, dynamic>.from(e as Map),
              ).copyWith(isSaved: true),
            )
            .toList();
  ...
  /// Fetch saved investors based on auth token
  ...
  Future<Result<Paginated<Founder>>> _fetchFounders(QueryParams params) async {
    ...
    return client.getEnvelope<Paginated<Founder>>(
      ApiEndpoints.publicSavedFounders,
      query: query,
      parser: (env) {
        final list = env.data as List? ?? [];
        final items = list
              ...
              IconButton(
                ...
                onPressed: () async {
                  final result = await sl<ApiClientHelper>().postAction(
                    '${ApiEndpoints.favorites}/toggle',
                    body: {'entityType': 'founder', 'entityId': f.id},
                  );
